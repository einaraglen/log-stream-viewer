...
class IndexableUtils {
  ...
  public query(table: string, range: Interval, type: Query): Promise<number[]> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const request = type === Query.Values ? store.getAll(bounds) : store.getAllKeys(bounds)
  ...
  public cache(table: number | string, range: Interval): Promise<Interval[]> {
    return new Promise(async (resolve, reject) => {
